//! SHA-512 (FIPS 180-4) — used by Ed25519 (RFC 8032) for key/prefix hashing
//! and message challenges. Pure `no_std`, zero `unsafe`.
...
impl Sha512 {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        let mut off = 0usize;
        if self.n > 0 && self.n + data.len() >= 128 {
            let take = 128 - self.n;
        ...
        while off + 128 <= data.len() {
            let mut b = [0u8; 128];
            b.copy_from_slice(&data[off..off + 128]);
    ...
    /// Finalize into a 64-byte digest.
